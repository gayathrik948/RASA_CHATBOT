...
/** This method will appened the dependent source js files link to the head element */
function addAutoChip() {
  /* style for autochip*/
  const autochipCss = document.createElement('link')
  autochipCss.setAttribute('rel', 'stylesheet')
  autochipCss.setAttribute('href', 'http://code.jquery.com/ui/1.10.4/themes/ui-lightness/jquery-ui.css');
  document.head.appendChild(autochipCss);

  const chatCss = document.createElement('link')
  chatCss.setAttribute('rel', 'stylesheet')
  chatCss.setAttribute('href', 'style.css');
  document.head.appendChild(chatCss);

  const chatJS = document.createElement('script')
  chatJS.setAttribute('src', 'autocomplete.multiselect.js');
...
function createChatConatiner () {
  ...
  chatheader.innerText = 'HR Portal Chatbot'
...
function botInteraction (data) {
  const payload = data.isMultiChip ? { message: data.message, sender: createUUID, metadata: data.metadata, user_id: 'B16343' }:{ message: data, sender: createUUID, user_id: 'B16343' } ;
  fetch(url + 'webhook', {
    method: 'POST',
...
function createDomSuggestion (response) {
  if (response.length > 0) {
    for (let i = 0; i < response.length; i++) {
      ...
      if (response[i].custom && response[i].custom.multi_select_chips === true) {
        const userDiV = document.getElementById('xpms-chat-container')

        const grpDiv = document.createElement('div');
        grpDiv.setAttribute('id', 'grpDivautocomplete' + i);
        grpDiv.setAttribute('class', 'grpDivautocomplete bot-chat talk-bubble bot-round btm-left-in');

        const autoinput = document.createElement('input');
        autoinput.setAttribute('id', 'autocompleteMultiple' + i);
        autoinput.setAttribute('type', 'text');
        autoinput.setAttribute('class', 'chipcls');
        autoinput.setAttribute('placeholder', 'type to search..');


        const grpinput = document.createElement('input');
        grpinput.setAttribute('id', 'inputgroup' + i);
        grpinput.setAttribute('type', 'text');
        grpinput.setAttribute('class', 'inputcls');
        grpinput.setAttribute('placeholder', 'optional comments');

        const grpBtn = document.createElement('button');
        grpBtn.setAttribute('class', 'child-button round');
        grpBtn.setAttribute('id', 'child-button-key');
        grpBtn.innerText = 'Submit';
        grpBtn.setAttribute('payload', ['autocompleteMultiple' + i, 'inputgroup' + i, 'grpDivautocomplete' + i]);

        grpBtn.addEventListener('click', onChipSubmit)
        const botMsg = document.createElement('span');
        botMsg.innerText = response[i].custom.message;

        grpDiv.appendChild(botMsg)

        grpDiv.appendChild(autoinput);
        grpDiv.appendChild(grpinput);
        grpDiv.appendChild(grpBtn);

        userDiV.appendChild(grpDiv);

        setTimeout(() => {
          $('#autocompleteMultiple' + i).autocomplete({
            source: response[i].custom.options,
            multiselect: true
          });
        }, 300);
...
/** This method will do the action as chip autocomplete */
function onChipSubmit(e) {
//  console.log(e);
  var selectedItemsarray = [];
  var list = document.getElementsByClassName('ui-autocomplete-multiselect-item');

  for (let item of list) {
    selectedItemsarray.push(item.innerText);
  }
  let inputgrpIds = e.target.getAttribute('payload').split(',')[1];
  let comments = document.getElementById(inputgrpIds).value;
  var data = {"message":"", "sender_id":createUUID,'isMultiChip':true, "metadata":{"select_options":selectedItemsarray, "comment":comments}};
  botInteraction(data);
  addUserChatText("selected Items:"+selectedItemsarray.join(',') +  '; </br>  comments: '+comments);
  let grpIds = e.target.getAttribute('payload').split(',')[2];
  let removedNode = document.getElementById(grpIds);

  const userDiV = document.getElementById('xpms-chat-container');
  userDiV.removeChild(removedNode);
  }
